...
?>
	...
	<?if ($arItem["IS_PARENT"]):?>
		<li>
					<a href="<?=$arItem["LINK"]?>"<?if($arItem["SELECTED"] && $arItem["LINK"] == $APPLICATION->GetCurDir()) echo ' style="background: #0975e9;color: #ffffff;padding: 6px 6px;width: 95%;"';?>><?=$arItem["TEXT"]?></a>
					<?if($arItem["SELECTED"]){?>
					<ul>
					<?}else{?>
