...
$res = CIBlockElement::GetList(false, Array("IBLOCK_ID" => 1, 'ACTIVE' => 'Y', "SECTION_ID" => array(2)), array('IBLOCK_ID')); 
...
if(!empty($mypage)) {
	$APPLICATION->SetPageProperty("title", "Технический надзор – примеры работ компании СтройЭкспертНадзор - страница №" . ($mypage + 1));
	$h1 = '<h1 class="title">Технический надзор - примеры работ - страница №' . ($mypage + 1) . '</h1>';
	$sum_prev = $mypage - 1;
	...
	$APPLICATION->SetPageProperty("title", "Технический надзор – примеры работ компании СтройЭкспертНадзор");
	$APPLICATION->SetPageProperty("description", "Портфолио компании СтройЭкспертНадзор в области Технический надзор с подробным описанием объектов и фото.");
	$prev = "";
	$h1 = '<h1 class="title">Технический надзор - примеры работ</h1>';
	$sum_next = $mypage + 1;
...
<section class="breadcrumbs">
	...
	<section class="sect6">
		...
		<div id="portfolio_usluga" style="display:none;">2</div>
...
list($pagertop, $pagerbottom, $limit) = pager(6, $count_objects, "/portfolio/tehnadzor/" );
...
$rsElement = CIBlockElement::GetList( array("ID" => "DESC"), Array("IBLOCK_ID" => 1, 'ACTIVE' => 'Y', "SECTION_ID" => array(2)), false,Array("nPageSize"=> 6, "iNumPage" => $numpage), $arSelect );
